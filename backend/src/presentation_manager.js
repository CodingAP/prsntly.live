        ...
        PresentationManager.io.on('connection', socket => {
            socket.on('RECONNECT_PRESENTATION', (code, uuid, callback) => {
                ...
                if (currentPresentation.connections.has(uuid)) {
                    ...
                    currentPresentation.globalState.pointers[newUUID] = null;
                    for (let i = 0; i < currentPresentation.globalState.drawingSurfaces.length; i++) {
                        currentPresentation.globalState.drawingSurfaces[i].current[newUUID] = null;
                    }
            ...
            socket.on('SEND_DRAW', (drawable, code) => {
                const state = PresentationManager.presentations.get(code).globalState;
                const currentSlideState = state.drawingSurfaces[state.currentSlide];
                if (drawable == null) {
                    if (currentSlideState.current[socket.uuid].type == 'RECTANGLE') {
                        if (currentSlideState.current[socket.uuid].data.start.x != currentSlideState.current[socket.uuid].data.end.x &&
                            currentSlideState.current[socket.uuid].data.start.y != currentSlideState.current[socket.uuid].data.end.y) {
                            currentSlideState.drawables.push(currentSlideState.current[socket.uuid]);
                        }
                    } else if (currentSlideState.current[socket.uuid].type != 'ERASER') {
                        if (currentSlideState.current[socket.uuid].data.length != 0) {
                            currentSlideState.drawables.push(currentSlideState.current[socket.uuid]);
                        }
                    }
                } else if (drawable.type == 'ERASER') {
                    // TODO: erase closest drawable
                }

                currentSlideState.current[socket.uuid] = drawable;
                PresentationManager.io.to(code).emit('GLOBAL_UPDATE', state);
            });

            socket.on('SEND_UNDO', code => {
                const state = PresentationManager.presentations.get(code).globalState;
                const currentSlideState = state.drawingSurfaces[state.currentSlide];
                
                for (let i = currentSlideState.drawables.length - 1; i >= 0; i--) {
                    if (currentSlideState.drawables[i].uuid == socket.uuid) {
                        currentSlideState.drawables.splice(i, 1);
                        break;
                    }
                }
            ...
            socket.on('SEND_POINTER', (pointer, code) => {
                const state = PresentationManager.presentations.get(code).globalState;
                state.pointers[socket.uuid] = pointer;
                PresentationManager.io.to(code).emit('GLOBAL_UPDATE', state);
            });
